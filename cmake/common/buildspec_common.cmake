...
# _setup_obs_studio: Build libobs and obs-frontend-api from downloaded source
function(_setup_obs_studio)
  ...
  foreach(_config IN ITEMS Debug Release)
    ...
    execute_process(
      COMMAND
        "${CMAKE_COMMAND}" --build "build_${arch}"
        --target obs-frontend-api --config ${_config} --parallel
    ...
    message(STATUS "Install ${label} (${_config} - ${arch})")
    execute_process(
      COMMAND
        "${CMAKE_COMMAND}" --install "build_${arch}"
        --component Development --config ${_config}
    ...
    message(STATUS "Install ${label} (${_config} - ${arch}) - done")
  endforeach()
endfunction()

# _check_dependencies: Download and set up prebuilt OBS deps, Qt6, and OBS source
function(_check_dependencies)
  ...
  foreach(dependency IN LISTS dependencies_list)
    ...
    if(skip)
      message(STATUS "Setting up ${label} (${arch}) - skipped (already up to date)")
    ...
    if(NOT EXISTS "${dependencies_dir}/${file}")
      ...
      file(DOWNLOAD "${url}" "${dependencies_dir}/${file}"
           STATUS download_status EXPECTED_HASH SHA256=${hash})
      list(GET download_status 0 error_code)
      list(GET download_status 1 error_message)
      if(error_code GREATER 0)
        file(REMOVE "${dependencies_dir}/${file}")
        message(FATAL_ERROR "Download failed for ${url}: ${error_message}")
      endif()
    ...
    if(NOT EXISTS "${dependencies_dir}/${destination}")
      ...
      if(dependency STREQUAL obs-studio)
        ...
        file(ARCHIVE_EXTRACT INPUT "${dependencies_dir}/${file}"

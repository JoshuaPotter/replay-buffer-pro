...
**Note:** Windows builds are 64-bit only, as OBS Studio 29.0.0+ dropped 32-bit support. macOS builds are universal binaries (arm64 + x86_64). Linux builds are available for Ubuntu 24.04+ (x86_64).
...
- CMake 3.28+, Ninja, pkg-config
- OBS Studio and FFmpeg installed from the OBS PPA

No manual OBS clone, Qt6 install, or FFmpeg setup is needed on Windows or macOS — everything is fetched automatically. On Linux, OBS and FFmpeg are installed from the system package manager.
...
# Configure (uses system OBS and FFmpeg from apt)
...
Pushing a semver tag (e.g., `1.4.0`) to `main`/`master` triggers the GitHub Actions workflow, which builds the plugin for Windows, macOS, and Linux, and creates a draft GitHub release with all artifacts attached.
...
- Verify plugin file location (`.dll` on Windows, `.plugin` bundle on macOS, `.so` on Linux)
...
- When building from source:
  ...
  - **Linux**: Ensure OBS Studio is installed from the OBS PPA (`sudo add-apt-repository ppa:obsproject/obs-studio`)
  ...
  - **Linux**: Install may require `sudo` for system-wide installation to `/usr`
